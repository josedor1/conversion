...
pdfFileobj = open('Carta.pdf', 'rb')  # Abra el archivo en formato binario de solo lectura
...
pdfFileobj.close()
f.close()
palabras = ['RESOLUCION', 'RESUELVE']
for i in palabras:
    print (palabras[i])
    p = open('borrador.txt', 'r').read().find(palabras[i])  # abrimos el archivo para lectura de informacion
